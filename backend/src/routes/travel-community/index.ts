...
function mapPostToCard(post: any) {
  ...
  return {
    ...
    type: DB_TYPE_TO_FRONT[post.postType] ?? "遊記",
    cover: firstMedia?.mediaUrl ?? "/travel-community/placeholder.png",
    duration: firstMedia?.mediaType === "video" && firstMedia?.durationSeconds
      ? `${Math.floor(firstMedia.durationSeconds / 60)}:${String(firstMedia.durationSeconds % 60).padStart(2, "0")}`
      : undefined,
    location: post.location ?? "未知地點",
    country: post.location?.slice(0, 2).toUpperCase() ?? "",
