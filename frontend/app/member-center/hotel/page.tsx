    ...
    { key: "paymentMethod", title: "付款方式", width: 100, align: "center" },
    {
      ...
      render: (value?: string) => {
        const normalize = (v: string): "success" | "disabled" | "refunding" | "refunded" => {
          ...
          if (["success", "done", "completed", "paid"].includes(v2)) return "success";
          if (["disabled", "cancel", "cancelled", "canceled", "fail", "failed"].includes(v2))
            return "disabled";
          if (["refunding", "processing", "pending_refund", "on_refund", "processing_refund"].includes(v2))
            return "refunding";
          if (["refunded", "refund_done", "finish_refund", "refund_success"].includes(v2))
            return "refunded";
