    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="mx-auto px-4 lg:px-16 max-w-4xl">
        ...
        <div className="bg-white rounded-lg p-6 md:p-12 text-center">
          ...
          <h1 style={{ fontSize: 'clamp(1.5rem, 5vw, 2rem)', lineHeight: '1.5', fontWeight: '600' }} className="mb-4">
            訂單完成！
          </h1>
          
          <p className="text-gray-600 mb-2 text-sm md:text-base">感謝您的訂購</p>
          <p className="text-gray-600 mb-6 md:mb-8 text-sm md:text-base">訂單號碼：{orderNumber}</p>
          ...
          <div className="mb-6 md:mb-8">
            ...
            <div className="flex flex-wrap justify-center gap-3 md:gap-4">
              {cart.map((item) => (
                <div key={item.id} className="w-20 h-20 md:w-24 md:h-24 bg-gray-100 rounded-lg overflow-hidden">
                  <ImageWithFallback
                    src={item.image}
                    alt={item.name}
          ...
          <div className="max-w-md mx-auto mb-6 md:mb-8 p-4 md:p-6 bg-[var(--df-surface-alt)] rounded-lg">
            <div className="flex justify-between mb-2 text-sm md:text-base">
              ...
              <span className="font-semibold">TWD {total.toLocaleString()}</span>
            </div>
          </div>
